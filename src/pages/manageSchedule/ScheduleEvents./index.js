      ...
      if (response.status === 200) {
        setEvents(
          response.data.map((data) => {
            return {
              ...data,
              weekFormat: data.week_days.reduce(
                (accumlator, currentValor, index) => {
                  let dayStr = '';
              ...
              hoursStartFormat: data.hours_start
                ? hourFormat(data.hours_start, false)
                : '',
              hoursEndFormat: data.hours_end
                ? hourFormat(data.hours_end, false)
                : '',
              ...
              renderItem={({ item }) => (
                <Line key={item.id}>
                  ...
                  <LineButton
                    color="#4da6ff"
                    onPress={() =>
                      navigation.navigate('CreateScheduleEvents', {
                        event: item,
