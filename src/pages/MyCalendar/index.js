...
export default function MyCalendar() {
      ...
      if (response.status === 200) {
        setMyAppointments(
          response.data.map((data) => ({
            ...
            idDate: format(
              new Date(data.date.substring(0, 4), data.date.substring(4, 7), 1),
              'yyyyMM'
            ),
          }))
        );
  ...
  async function loadingAppointments(year, month, idDate, refresh = false) {
    ...
    if (selectedMonth !== idDate || refresh === true) {
      const response = await api.get('appointments/months/', {
        params: { providerId: user.id, year, month },
      });
      ...
      if (response.status === 200) {
        ...
        if (!refresh) setSelectedMonth(idDate);
      }
    } else {
      setAppointments([]);
              ...
              setAppointments(
                appointments.map((appointment) => {
                  return {
                    ...appointment,
                    status:
                      appointment.id === id ? 'schedule' : appointment.status,
                    loose_client:
                      appointment.id === id
                        ? DEFAULT_CLIENTE
            ...
            <ListHorizontal
              ...
              renderItem={({ item }) => (
                <ButtonCardMonth
                  key={item.date}
                  onPress={() =>
                    loadingAppointments(item.year, item.month, item.idDate)
                  }
                ...
                >
                  ...
                  <CardColumn>
                    ...
                    <TextVacancies>
                      {item.vacancies_morning} Manhã
                    </TextVacancies>
                    <TextVacancies>
                      {item.vacancies_afternoon} Tarde
                    </TextVacancies>
