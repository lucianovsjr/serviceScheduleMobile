...
export const dateFormat = (date, req = true, reqDate = false) => {
  let returnDate = date;

  if (typeof date === 'string') returnDate = parseISO(date);

  if (returnDate.getFullYear() === 1969) return null;

  if (reqDate) return returnDate;

  if (req) return format(returnDate, 'yyyy-MM-dd');
...
export const hourFormat = (hour, req = true, reqDate = false) => {
  ...
  if (reqDate) return returnHour;

  if (req) return format(returnHour, 'HH:mm:ss', { locale: pt });
...
export const nameMonthFormat = (date) => {
  ...
  return format(new Date(year, month, 1), 'MMM/yyyy', { locale: pt });
};
